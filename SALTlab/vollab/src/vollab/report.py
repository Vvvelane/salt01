...
def _limits() -> list[str]:
    return [
        ...
        "就要整份重跑，不能和旧版混着看。",
        "- **派生数据的新鲜度目前是不齐的**：主连 1min 有 72 个品种停在 2026-08-07，"
        "只有 10 个到 2026-09-07；全部合约 1min 有 61 个到 2026-09-07，郑商所整体停在 2026-08-06。"
        "本排名的窗口是三年，尾端差一个月对分数的影响可以忽略，"
        "而且「交易日内部缺口」一项只在品种自己的首末日之间算，不会把派生任务的排期问题"
        "记成品种的数据质量问题。但如果要拿这些数据做实盘对齐，就得先把派生任务补齐。",
        "- `元数据/catalog.duckdb` 里的 `row_count` / `max_time` 是旧快照，不可信；"
